...
class _MyOrdersPageState extends State<MyOrdersPage>
  ...
  final _orders = [
    _Order(id: '#VLV7842', date: 'May 20, 2026', total: 288.00,
        status: 'delivered', items: 2,
        imageUrl: 'https://picsum.photos/seed/ord1/200/200'),
    _Order(id: '#VLV7831', date: 'May 15, 2026', total: 199.00,
        status: 'shipped', items: 1,
        imageUrl: 'https://picsum.photos/seed/ord2/200/200'),
    _Order(id: '#VLV7820', date: 'May 10, 2026', total: 89.00,
        status: 'processing', items: 1,
        imageUrl: 'https://picsum.photos/seed/ord3/200/200'),
    _Order(id: '#VLV7810', date: 'Apr 28, 2026', total: 350.00,
        status: 'cancelled', items: 3,
        imageUrl: 'https://picsum.photos/seed/ord4/200/200'),
  ];

  List<_Order> _filtered(String tab) {
    if (tab == 'All') return _orders;
    if (tab == 'Active') return _orders.where((o) =>
        o.status == 'processing' || o.status == 'shipped').toList();
    if (tab == 'Completed') return _orders.where((o) =>
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          unselectedLabelStyle: AppTextStyle.s12w4(color: AppPallete.subTextColor),
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: AppPallete.background, shape: BoxShape.circle),
                    child: Icon(Icons.receipt_long_outlined,
                        size: 36, color: AppPallete.primary),
                  ),
...
class _OrderCard extends StatelessWidget {
  ...
  Color _statusColor() {
    switch (order.status) {
      case 'delivered':  return AppPallete.success;
      case 'shipped':    return Colors.blue;
      case 'processing': return AppPallete.star;
      case 'cancelled':  return AppPallete.error;
      default:           return AppPallete.subTextColor;
    }
  }
  ...
  IconData _statusIcon() {
    switch (order.status) {
      ...
      case 'processing': return Icons.pending_outlined;
              ...
              ClipRRect(
                ...
                child: Image.network(order.imageUrl,
                    ...
                    errorBuilder: (_, __, ___) => Container(
                      width: 64, height: 64, color: AppPallete.background)),
              ),
...
class _Order {
  ...
  final double total;
  final int items;
  const _Order({required this.id, required this.date, required this.total,
      required this.status, required this.items, required this.imageUrl});
}
